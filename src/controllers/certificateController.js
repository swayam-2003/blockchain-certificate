...
class CertificateController {
    async generateCertificate(req, res) {
        try {
            const { studentName, courseName, validUntil } = req.body;

            // Input validation
            if (!studentName || !courseName || !validUntil) {
                return res.status(400).json({
                    success: false,
                    message: 'Missing required fields: studentName, courseName, and validUntil are required'
                });
            }

            console.log('Generate certificate request received:', { studentName, courseName, validUntil });
